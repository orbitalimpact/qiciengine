...
qici.init = function() {
    ...
    var loadingInterval = loadingConfig.loadingInterval;
    var brightingInterval = loadingConfig.brightingInterval;
    var blinkingCount = loadingConfig.blinkingCount;
    var blinkingInterval = loadingConfig.blinkingInterval;
    var fadingInInterval = loadingConfig.fadingInInterval;
    var fadingOutInterval = loadingConfig.fadingOutInterval;

    var loadState = 'loading';
    var tickIndex = -1;
    var lastTime = null;
    var startFadingInTime = null;
    var startFadingOutTime = null;
    var targetRotate;
    var targetX;
    var targetY;
    ...
    // set loading text
    document.getElementById('loadingText').textContent = loadingConfig.loadingText || '';
    ...
    function blinkDot() {
        ...
        if (blinkingCount === 0) {
            ...
            // begin to show logo
            loadState = 'fadingIn';
            startFadingInTime = new Date().getTime();
        }
        else {
    ...
    function fadingIn() {
        ...
        var delta = time - startFadingInTime;

        var rate = delta / fadingInInterval;
        if (rate > 1.5) {
            loadState = 'fadingOut';
            startFadingOutTime = new Date().getTime();
    ...
    function fadingOut() {
        var time = new Date().getTime();
        var delta = time - startFadingOutTime;

        var rate = delta / fadingOutInterval;
        if (rate > 1) {
            // finish fade in
            loadState = 'done';
    ...
    function tick() {
        ...
        else if (loadState === 'fadingIn') {
            fadingIn();
        }
        else if (loadState === 'fadingOut') {
            fadingOut();
        }
